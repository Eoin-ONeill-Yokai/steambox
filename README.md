...
You should create your distrobox with the following comman. **Note:** For NVidia graphics card users, you will need to pass the `--nvidia` argument when calling `distrobox create` (this is necessary to pass the graphics drivers into the guest system).

```
distrobox create --image ghcr.io/eoin-oneill-yokai/steambox:stable --init-hooks "install -o 1000 -g 1000 -d /tmp/.X11-steam-alt; mount --bind /tmp/.X11-steam-alt /tmp/.X11-unix" --pull always --home ~/.steambox steambox 
```
